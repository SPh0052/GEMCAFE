...
class VerificationListItem(BaseModel):
    ...
    videoWatermarkedId: int | None = Field(None, description="대상 워터마크 영상 ID (NOT_DETECTED 시 null 가능)")
    status: str = Field(..., description="검출 상태 (DETECTED/NOT_DETECTED/PROCESSING)")
    originalFileName: str = Field("", description="원본 파일명")
    fileSize: int = Field(0, description="파일 크기 (bytes)")
    durationSec: float = Field(0.0, description="영상 길이 (초)")
