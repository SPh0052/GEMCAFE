...
- 디저트: 딸기 생크림 조각 케이크
- 강조 요소: 생크림
- 시뮬레이션: 반으로 자르기 (칼이 케이크 조각을 수직으로 갈라 단면 노출)
- 배경: 흰 대리석
- 사장님 추가: "고급스럽게"
- 디저트 요소별 예상 질감:
...
좋은 출력: "흰 대리석 위에 단정하게 놓인 딸기 생크림 조각 케이크. 은빛 칼날이 위에서 수직으로 천천히 내려와 케이크를 반으로 가르자, 부드러운 생크림이 결 따라 매끄럽게 분리되고 시트의 결과 딸기 단면이 드러납니다. 차분하고 고급스러운 분위기."

# 예시 2 — 포크로 한 입 뜨기 (단면 노출 동반)
입력:
- 디저트: 딸기 생크림 조각 케이크
- 강조 요소: 생크림
- 시뮬레이션: 포크로 한 입 뜨기 (포크가 케이크 한 입 분량을 떠 들어올림)
- 배경: 어두운 무드
- 사장님 추가: 없음
- 디저트 요소별 예상 질감:
  - 생크림: 부드럽게 갈라짐, 결 따라 매끄럽게 분리
...
좋은 출력: "어두운 슬레이트 위 딸기 생크림 조각 케이크. 포크가 케이크 앞쪽에서 한 입 분량을 천천히 들어올리자 부드러운 생크림이 결 따라 매끄럽게 늘어나고, 들린 조각의 단면에서 시트의 결과 딸기 단면이 드러납니다. 측면 조명이 주는 무게감 있는 분위기."

# 예시 3 — 토핑 안착 (역방향 시뮬레이션)
입력:
- 디저트: 딸기 생크림 조각 케이크
- 강조 요소: 딸기
- 시뮬레이션: 딸기가 케이크 위로 톡 떨어진다 (한 알이 위에서 떨어져 안착)
- 배경: 원목 테이블
- 사장님 추가: "신선하게"
- 디저트 요소별 예상 질감: (안착 액션 — 별도 가이드 없음)

좋은 출력: "원목 테이블 위 부드러운 생크림이 덮인 케이크. 잘 익은 딸기 한 알이 위에서 톡 떨어져 케이크 위 자기 자리에 살짝 튕기며 안착합니다. 따스한 오후 햇살이 딸기의 신선한 광택을 비춥니다."
...
if __name__ == "__main__":
    ...
    texture_guidance = prompt_locks.get_texture_guidance(elements, "cut_in_half")
    print(f"[texture_guidance]\n{texture_guidance}\n")

    korean = generate_korean_preview(
        dessert_info="딸기 쇼트케이크",
        focus_label_kr="신선한 딸기",
        simulation_label_kr=prompt_locks.get_simulation_label_kr("cut_in_half"),
