...
Gemini 2.5 Pro (SSAFY GMS 게이트웨이 경유) 로 케이크 구성요소 + 5축 식감 분석.

**함수**
| 함수 | 역할 | 반환 |
|---|---|---|
| `_normalize_image_for_gemini(path, max_edge)` | EXIF 회전 보정 · RGB 변환 · 리사이즈 · JPEG 재인코딩 | `(bytes, "image/jpeg")` |
| `analyze_with_gemini(image_path, prompt)` | Gemini 2.5 Pro 멀티모달 호출 + JSON 파싱 (1024→768→512px fallback) | `(parsed_dict, raw_text, full_response)` |
| `_augment_creamy_bases(analysis)` | 후처리 — 바스크·무스 같은 크리미한 base 를 creams 에도 미러링 (FE 카테고리 필터 통과용) | `dict` |
| `extract_json(text)` | 모델 응답에서 JSON 부분만 추출 | `dict` |

**상수**
| 이름 | 역할 | 수정 빈도 |
|---|---|---|
| `GEMINI_VISION_MODEL` | Gemini 모델 ID (`"gemini-2.5-pro"`) | 거의 X |
| `FALLBACK_LONG_EDGE_PX` | 게이트웨이 거부 회피용 다운스케일 폴백 체인 (`[1024, 768, 512]`) | 거의 X |
| `INPUT_IMAGE_PATH` | 단독 실행 시 입력 이미지 경로 | 테스트마다 |
| `ANALYSIS_PROMPT` | few-shot 시스템 프롬프트 (6개 예시 + element_textures 5축 규칙 + visual cues + anti-copy) | 분석 정확도 튜닝 시 |
...
| `collect_elements_from_analysis(analysis)` | Gemini 분석 결과 → 요소 키 리스트 |
| `collect_elements_by_role(analysis)` | 분석 결과 → 역할별(base/cream/topping/coating) 요소 dict |
| `get_cake_structure_context_en(analysis)` | 역할별 visual_identity_en 묶음 → I2I 프롬프트 prepend용 영어 컨텍스트 |
| `get_signature_feature_en(analysis)` | `analysis.key_feature` → "Signature visual to emphasize: ..." 한 줄 영어 |
| `get_temperature_note_en(analysis)` | `analysis.is_warm=True` → 따뜻한 디저트 가이드(증기·점성 흐름) 한 줄 영어 |
...
| `TEXTURE_PROFILES` | dict | 요소별 질감/반응 (한국어 `under_pressure_kr`/`when_cut_kr` + 영어 `under_pressure_en`/`when_cut_en` + `visual_identity_en`) |
| `SIMULATION_ACTION_TYPE` | dict | 시뮬레이션 → 어느 질감 컬럼 사용할지 (`"under_pressure"` / `"when_cut"` / `None`) |
| `ELEMENT_ALIASES` | dict | 분석 모델이 변종 키로 줄 때 정식 키로 정규화 (예: `fluffy_whipped_cream` → `whipped_cream`) |
...
"칼로 단면 가르기"  ←→  "cut_in_half"   ←→   "A cake knife descends..."
```

### 2.1 시뮬레이션 ID 변환표

| AI 코드 (snake_case) | 한국어 라벨 (FE) | 카테고리 | action_type | frame_strategy |
|---|---|---|---|---|
| `smash` | 뭉개기 | 크림 | `under_pressure` | `i2i_is_end` |
| `fork_bite` | 포크로 한 입 뜨기 | 시트 | `when_cut` | `i2i_is_end` |
| `cut_in_half` | 칼로 단면 가르기 | 시트 | `when_cut` | `i2i_is_end` |
| `lift_slice` | 한 조각 쏙 들어올리기 | 시트 | `when_cut` | `i2i_is_end` |
| `cream_scoop` | 크림만 떠내기 | 크림 | `under_pressure` | `i2i_is_end` |
| `hand_split` | 손으로 반 가르기 | 크림 | `when_cut` | `i2i_is_end` |
| `topping_fall` | 위에서 떨어뜨리기 | 토핑 | `None` (질감 가이드 불필요) | `i2i_is_start` |

**정의 위치:** [prompt_locks.py](./prompt_locks.py)의 `SIMULATION_LABELS_KR`, `CAMERA_DIRECTIVES`, `SIMULATION_ACTION_TYPE` + [prompt_builder.py](./prompt_builder.py)의 `SIMULATIONS` (카테고리·frame_strategy·instruction_template·video_template)

---

### 2.2 Focus 키 변환표

| AI 코드 (Gemini Vision / snake_case) | 한국어 라벨 | 영어 (FOCUS_TEXT) | 질감 (when_cut_kr) |
|---|---|---|---|
...
| 값 | 의미 | 적용 시뮬레이션 | 영상 흐름 |
|---|---|---|---|
| `i2i_is_end` | I2I 결과 = 영상의 **끝** 프레임 | `smash`, `fork_bite`, `cut_in_half`, `lift_slice`, `cream_scoop`, `hand_split` | 원본 → I2I 결과 (변형 후) |
...
### 2.6 Gemini Vision 분석 결과 → AI 내부 키 매핑

[analyze.py](./analyze.py)가 Gemini 2.5 Pro 에서 받는 JSON의 어떤 필드가 AI 내부에서 어떻게 쓰이는지.

| 분석 출력 필드 | 타입 | 사용 위치 |
|---|---|---|
| `cake_type` | str | (현재 미사용 — 향후 디저트 분류용. 분석 결과 안에는 보존) |
| `base` | list | `collect_elements_by_role()` → 슬롯/카테고리·structure_ctx·creamy base 미러링 |
| `creams` | list | 동일 + `cream` 슬롯 채움 / `_augment_creamy_bases()` 결과 포함 |
| `toppings` | list | 동일 + `topping` 슬롯 채움 |
| `coating` | str | `collect_elements_by_role()` ("none" 제외) |
| `key_feature` | str | `get_signature_feature_en()` → I2I + I2V 프롬프트 prepend ("Signature visual to emphasize: ...") |
| `is_warm` | bool | `get_temperature_note_en()` → True 일 때 따뜻한 디저트 가이드 prepend (증기·점성 흐름) |
| `is_layered` | bool | `_resolve_slot_phrases()` → `{interior_structure}` 슬롯 (다층 단면 vs 균일 단면) |
| `element_textures` | dict | `_resolve_slot_phrases()` → `{texture}` 슬롯의 modifier (2-레이어 텍스처 합성의 사진별 식감) |
| `suggested_focus` | list | `derive_focus_from_category()` 폴백 — 카테고리 기반 결정 실패 시 `suggested_focus[0]` |

**정의 위치:** [analyze.py](./analyze.py)의 `ANALYSIS_PROMPT` (6개 few-shot + 5축 element_textures 규칙 + visual cues + anti-copy)
...
#### 새 focus 요소 추가 (Gemini Vision이 잡는 새 재료)
- [prompt_builder.py](./prompt_builder.py)의 `FOCUS_TEXT` (영어 자연어)
- [prompt_locks.py](./prompt_locks.py)의 `TEXTURE_PROFILES` (한국어/영어 질감 + visual_identity_en)
- [analyze.py](./analyze.py)의 `ANALYSIS_PROMPT` ELEMENT NAMING RULES 에 canonical 키 추가
...
| Gemini Vision 분석 결과의 `cake_type` | [analyze.py](./analyze.py) `ANALYSIS_PROMPT` | 추출은 되나 프롬프트 빌드 단계에서 사용 안 됨. | 향후 디저트 종류별 시뮬레이션 자동 추천에 활용 가능. |

> 과거에 `key_feature` / `is_warm` / `is_layered` / `element_textures` 가 "추출만 되고 미활용" 상태였으나, 모두 메인 흐름에 연결 완료. (각각 `get_signature_feature_en`, `get_temperature_note_en`, `{interior_structure}` 슬롯, 2-레이어 텍스처 합성 modifier 로 활용 중 — 자세한 내용은 [2.6 Gemini Vision 분석 결과 → AI 내부 키 매핑](#26-gemini-vision-분석-결과--ai-내부-키-매핑) 참조.)
